...
def nspk_authentication(sock, server_name):
    ...
    if alice_resp_nonce == alice_nonce:
        ...
        request = rsa.encrypt(server_pkey, request)
        sock.sendall(request)
        print("Alice: sent {}'s nonce {}".format(server_name, server_nonce))

	# check if verified connection on both sides
        response = int(sock.recv(1024))
        if response == RESP_VERIFIED:
            print("Alice: {} verified me!".format(server_name))
        else:
            print("Alice: {} could not verify me, something went wrong".format(server_name))
    else:
